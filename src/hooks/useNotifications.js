...
export const useNotifications = (options = {}) => {
  ...
  const loadNotifications = useCallback(async (params = {}) => {
    ...
    try {
      setPagination(currentPagination => {
        ...
        // 异步执行 API 调用
        notificationApi.getNotifications(queryParams).then(response => {
          if (response.success) {
   ...
   */
  const filterByType = useCallback(async (filterType) => {
   ...
   */
  const toggleUnreadOnly = useCallback(async (showUnreadOnly) => {
